...
export default function EditEquipmentPage() {
  ...
  useEffect(() => {
    // Загружаем данные оборудования и список складов
    const loadData = async () => {
      try {
        const [equipmentRes, warehousesRes] = await Promise.all([
          fetch(`http://localhost:4000/api/equipment/${id}`),
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md mx-auto bg-white p-6 rounded-lg shadow-md">
      <h2 className="text-2xl font-bold mb-6">Редактировать оборудование</h2>
